...
const CalculadoraPT = ({ talla, peso, logic }) => {
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-lg">
      <h3 className="text-xl font-semibold mb-4 flex items-center gap-2"><Ruler /> Calculadora de Porcentaje Peso/Talla (%P/T)</h3>
...
const TablaIMC = ({ pesoPreGestacional, talla }) => {
    ...
    return ( <div className="bg-white p-6 rounded-lg shadow-lg"> <h3 className="text-xl font-semibold mb-4 flex items-center gap-2"><Table /> Ganancia de Peso Recomendada según IMC Pre-Gestacional</h3> <div className="mb-4 p-3 bg-indigo-50 rounded-lg"><p className="text-sm font-medium text-indigo-800">IMC Pre-Gestacional Calculado:</p><p className="text-2xl font-bold text-indigo-600">{imcPreGestacional > 0 ? imcPreGestacional.toFixed(2) : 'N/A'}</p></div> <div className="overflow-x-auto"><table className="w-full text-left border-collapse"><thead><tr className="bg-gray-200"><th className="p-3 font-semibold text-sm border">Categoría de peso basada en el IMC</th><th className="p-3 font-semibold text-sm border">Aumento de peso total al termino</th><th className="p-3 font-semibold text-sm border">Aumento durante el primer trimestre</th><th className="p-3 font-semibold text-sm border">Aumento semanal durante el 2º y 3er trimestre</th></tr></thead><tbody>{data.map((row) => (<tr key={row.key} className={`text-sm ${row.key === categoriaActual ? 'bg-green-100 font-bold' : 'bg-white'}`}><td className="p-3 border">{row.categoria}</td><td className="p-3 border">{row.total}</td><td className="p-3 border">{row.primerTrim}</td><td className="p-3 border">{row.semanal}</td></tr>))}</tbody></table><p className="text-xs text-gray-500 mt-2">Fuente: Kathleen M. Rasmussen and Ann L. Yaktine, Editors; Committee to Reexamine IOM Pregnancy Weight Guidelines; Institute of Medicine; National Research Council, 2009.</p></div> </div> );
};
...
const GyTTab = ({ patientData }) => {
  ...
  const SubTabButton = ({ tabId, label, icon }) => ( <button onClick={() => setActiveSubTab(tabId)} className={`flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-md transition-all ${activeSubTab === tabId ? 'bg-green-600 text-white shadow-md' : 'bg-white text-gray-700 hover:bg-gray-100'}`}> {icon} {label} </button> );
